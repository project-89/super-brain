...
| `GET` | `/v1/workspaces/:workspace/event-stream` | Resumable filtered SSE after an exclusive cursor |
| `GET`, `POST` | `/v1/workspaces/:workspace/consumers/:consumerId` | Principal-scoped durable consumer cursor |
...
| `GET`, `POST` | `/v1/workspaces/:workspace/memory-candidates` | Project-aware proposal review or creation |
| `POST` | `/v1/workspaces/:workspace/memory-candidate-imports` | Atomic proposal batches of at most 100 |
| `POST` | `/v1/workspaces/:workspace/memory-candidate-promotions` | Atomic accepted-memory batches of at most 100 |
...
workspace advisory lock for each append batch, and persists consumer offsets
and rebuildable projection checkpoints. JSONL remains available for local use,
...
recall and are partitioned by the authenticated workspace.
